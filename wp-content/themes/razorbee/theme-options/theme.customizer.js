/* global greenthumb_color_schemes, greenthumb_dependencies, Color */
...
( function( api ) {
	...
	for (var i in greenthumb_color_schemes) {
		cssTemplate[i] = wp.template( 'greenthumb-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'greenthumb-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+greenthumb_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(greenthumb_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', greenthumb_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', greenthumb_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			greenthumb_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) greenthumb_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		greenthumb_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		greenthumb_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function greenthumb_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('greenthumb_options_no_use');
				} else {
					jQuery(this).hide().addClass('greenthumb_options_no_use');
				}
			}
		});
	}
	...
	function greenthumb_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in greenthumb_theme_fonts) {
					for (rule in greenthumb_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							greenthumb_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) greenthumb_customizer_update_css();
		}
	...
	// Actions buttons
	function greenthumb_customizer_actions(e) {
	...
	function greenthumb_customizer_update_theme_fonts(opt, rule, value) {
		greenthumb_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function greenthumb_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=greenthumb_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in greenthumb_theme_fonts) {
	...
	function greenthumb_customizer_update_css() {
		...
		var fonts = greenthumb_customizer_add_theme_fonts(greenthumb_theme_fonts);

		// Make styles and add into css
		css += greenthumb_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in greenthumb_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in greenthumb_color_schemes[scheme].colors) {
				colors[i] = greenthumb_color_schemes[scheme].colors[i];
			}
	...
	function greenthumb_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + greenthumb_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + greenthumb_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + greenthumb_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function greenthumb_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function greenthumb_customizer_prepare_html_value(val) {
