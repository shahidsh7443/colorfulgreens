...
if (!function_exists('greenthumb_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'greenthumb_hovers_theme_setup3', 3 );
	function greenthumb_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		greenthumb_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'greenthumb'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'greenthumb') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'greenthumb'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'greenthumb'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'greenthumb') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'greenthumb'),
					'icon'	=> esc_html__('Icon',	'greenthumb'),
					'icons'	=> esc_html__('Icons',	'greenthumb'),
					'zoom'	=> esc_html__('Zoom',	'greenthumb'),
					'fade'	=> esc_html__('Fade',	'greenthumb'),
					'slide'	=> esc_html__('Slide',	'greenthumb'),
					'pull'	=> esc_html__('Pull',	'greenthumb'),
					'border'=> esc_html__('Border',	'greenthumb')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('greenthumb_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'greenthumb_hovers_theme_setup9', 9 );
	function greenthumb_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'greenthumb_hovers_frontend_scripts', 1010 );
		add_filter( 'greenthumb_filter_localize_script','greenthumb_hovers_localize_script' );
		add_filter( 'greenthumb_filter_merge_scripts',	'greenthumb_hovers_merge_scripts' );
		add_filter( 'greenthumb_filter_merge_styles',	'greenthumb_hovers_merge_styles' );
		add_filter( 'greenthumb_filter_get_css', 		'greenthumb_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'greenthumb_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'greenthumb_hovers_frontend_scripts', 1010 );
	function greenthumb_hovers_frontend_scripts() {
		if ( greenthumb_is_on(greenthumb_get_theme_option('debug_mode')) && greenthumb_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'greenthumb-hovers', greenthumb_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( greenthumb_is_on(greenthumb_get_theme_option('debug_mode')) && greenthumb_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'greenthumb-hovers',  greenthumb_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('greenthumb_hovers_merge_scripts')) {
	//Handler of the add_filter( 'greenthumb_filter_merge_scripts', 'greenthumb_hovers_merge_scripts' );
	function greenthumb_hovers_merge_scripts($list) {
...
if (!function_exists('greenthumb_hovers_merge_styles')) {
	//Handler of the add_filter( 'greenthumb_filter_merge_styles', 'greenthumb_hovers_merge_styles' );
	function greenthumb_hovers_merge_styles($list) {
...
if (!function_exists('greenthumb_hovers_localize_script')) {
	//Handler of the add_filter( 'greenthumb_filter_localize_script','greenthumb_hovers_localize_script' );
	function greenthumb_hovers_localize_script($arr) {
		$arr['button_hover'] = greenthumb_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('greenthumb_hovers_add_icons') ) {
	function greenthumb_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'greenthumb') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'greenthumb');
						else
							esc_html_e('Details', 'greenthumb');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'greenthumb_hovers_get_css' ) ) {
	//Handler of the add_filter( 'greenthumb_filter_get_css', 'greenthumb_hovers_get_css', 10, 4 );
	function greenthumb_hovers_get_css($css, $colors, $fonts, $scheme='') {
