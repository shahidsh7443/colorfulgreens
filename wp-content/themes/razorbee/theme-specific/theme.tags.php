...
if ( !function_exists('greenthumb_need_page_title') ) {
	function greenthumb_need_page_title() {
		return !is_front_page() && apply_filters('greenthumb_filter_need_page_title', true);
	}
}
...
if ( !function_exists('greenthumb_show_layout') ) {
	function greenthumb_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('greenthumb_get_logo_image') ) {
	function greenthumb_get_logo_image($type='') {
		$logo_image = '';
		if (greenthumb_is_on(greenthumb_get_theme_option('logo_retina_enabled')) && greenthumb_get_retina_multiplier(2) > 1)
			$logo_image = greenthumb_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = greenthumb_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('greenthumb_get_header_video') ) {
	function greenthumb_get_header_video() {
		$video = '';
		if (apply_filters('greenthumb_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (greenthumb_check_theme_option('header_video')) {
				$video = greenthumb_get_theme_option('header_video');
...
if ( !function_exists('greenthumb_show_post_meta') ) {
	function greenthumb_show_post_meta($args=array()) {

		if (is_single() && greenthumb_is_off(greenthumb_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('greenthumb_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php greenthumb_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('greenthumb_filter_get_post_date', greenthumb_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( ($output = greenthumb_get_post_counters($args['counters'])) != '')
						greenthumb_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							$post_comments = get_comments_number();
							?>
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-icon-comment-3"><?php
								?><span class="post_counters_number"><?php
									echo esc_html($post_comments);
								?></span><?php
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'greenthumb'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					greenthumb_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'greenthumb'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'greenthumb' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('greenthumb_show_post_featured') ) {
	function greenthumb_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => greenthumb_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : greenthumb_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? greenthumb_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = greenthumb_get_post_image();
				if (!empty($image)) 
					$image = greenthumb_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . greenthumb_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 greenthumb_is_on(greenthumb_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$greenthumb_attr = greenthumb_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($greenthumb_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($greenthumb_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						greenthumb_hovers_add_icons($args['hover']);
					}
					...
					if (($output=greenthumb_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						greenthumb_show_layout($output);
					...
					if (!empty($image)) {
						...
						greenthumb_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = greenthumb_get_post_video($post_content, false);
						if (empty($video))
							$video = greenthumb_get_post_iframe($post_content, false);
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = greenthumb_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									greenthumb_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = greenthumb_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = greenthumb_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							//Show metadata (for the future version)
							if (false && function_exists('wp_read_audio_metadata')) {
								$src = greenthumb_get_post_audio($audio);
							...
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								...
								$media_author = greenthumb_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = greenthumb_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !greenthumb_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php greenthumb_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !greenthumb_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php greenthumb_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								greenthumb_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				greenthumb_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				greenthumb_show_layout($post_info);
...
if ( !function_exists('greenthumb_get_no_image') ) {
	function greenthumb_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = greenthumb_get_theme_option( 'no_image' );
			if (empty($img)) $img = greenthumb_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('greenthumb_add_bg_in_post_nav') ) {
	function greenthumb_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = greenthumb_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), greenthumb_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = '';
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), greenthumb_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = '';
			if ( !empty($img) )
		...
		wp_add_inline_style( 'greenthumb-main', $css );
	}
}

// Show related posts
if ( !function_exists('greenthumb_show_related_posts') ) {
	function greenthumb_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : greenthumb_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'greenthumb');
				?></h3>
...
if ( !function_exists('greenthumb_show_portfolio_posts') ) {
	function greenthumb_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? greenthumb_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = greenthumb_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = greenthumb_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the greenthumb_ajax_get_posts action
if ( !function_exists( 'greenthumb_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_greenthumb_ajax_get_posts',			'greenthumb_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_greenthumb_ajax_get_posts',	'greenthumb_ajax_get_posts_callback');
	function greenthumb_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			greenthumb_storage_set('blog_mode', 'blog');
			greenthumb_storage_set('options_meta', get_post_meta($id, 'greenthumb_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => greenthumb_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'greenthumb');
		}
...
// Show pagination
if ( !function_exists('greenthumb_show_pagination') ) {
	function greenthumb_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = greenthumb_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'greenthumb' ),
				'next_text' => esc_html__( '>', 'greenthumb' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'greenthumb' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'greenthumb'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'greenthumb') : esc_html__('Newest posts', 'greenthumb') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'greenthumb') : esc_html__('Older posts', 'greenthumb'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
