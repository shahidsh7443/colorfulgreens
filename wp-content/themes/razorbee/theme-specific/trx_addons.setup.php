...
------------------------------------------------------------------------------- */
if (!function_exists('greenthumb_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'greenthumb_trx_addons_theme_specific_components');
	function greenthumb_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('greenthumb_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'greenthumb_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'greenthumb_trx_addons_theme_specific_setup1', 1 );
	function greenthumb_trx_addons_theme_specific_setup1() {
		if (greenthumb_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'greenthumb_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'greenthumb_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'greenthumb_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'greenthumb_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'greenthumb_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'greenthumb_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'greenthumb_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'greenthumb_trx_addons_cv_enable');
	function greenthumb_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return GREENTHUMB_THEME_FREE
					? false		// Free version
					: true;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'greenthumb_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'greenthumb_trx_addons_demo_enable');
	function greenthumb_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'greenthumb_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'greenthumb_trx_addons_edd_themes_market_enable')) {
	function greenthumb_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'greenthumb_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'greenthumb_trx_addons_api_list');
	function greenthumb_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'greenthumb_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'greenthumb_trx_addons_cpt_list');
	function greenthumb_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'greenthumb_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'greenthumb_trx_addons_sc_list');
	function greenthumb_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'greenthumb_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'greenthumb_trx_addons_widgets_list');
	function greenthumb_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'greenthumb_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'greenthumb_trx_addons_menu_cache');
	function greenthumb_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('greenthumb_trx_addons_localize_script')) {
	...
	function greenthumb_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = greenthumb_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'greenthumb_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'greenthumb_trx_addons_sc_type', 10, 2);
	function greenthumb_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'greenthumb_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'greenthumb_trx_addons_sc_atts', 10, 2);
	function greenthumb_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'greenthumb_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'greenthumb_trx_addons_sc_map', 10, 2);
	function greenthumb_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'greenthumb'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'greenthumb') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(greenthumb_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'greenthumb_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'greenthumb_trx_addons_sow_map', 10, 2);
	function greenthumb_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'greenthumb'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'greenthumb') ),
				'options' => greenthumb_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			greenthumb_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			greenthumb_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'greenthumb_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'greenthumb_trx_addons_sc_output', 10, 4);
	function greenthumb_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !greenthumb_is_inherit($atts['color_style']))
...
if ( !function_exists( 'greenthumb_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'greenthumb_trx_addons_sc_item_title_tag');
	function greenthumb_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'greenthumb_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'greenthumb_trx_addons_sc_item_button_args', 10, 3);
	function greenthumb_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'greenthumb_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'greenthumb_trx_addons_slider_title', 10, 2 );
	function greenthumb_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'greenthumb_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'greenthumb_trx_addons_sc_googlemap_styles');
	function greenthumb_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'greenthumb');
		$list['light'] = esc_html__('Light', 'greenthumb');
		return $list;
	}
}
...
if ( !function_exists( 'greenthumb_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'greenthumb_trx_addons_tiny_mce_style_formats');
	function greenthumb_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'greenthumb') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'greenthumb'),
								'selector' => 'ul',
...
if ( !function_exists( 'greenthumb_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'greenthumb_filter_allow_override_header_image', 'greenthumb_trx_addons_allow_override_header_image' );
	function greenthumb_trx_addons_allow_override_header_image($allow) {
		return greenthumb_is_team_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'greenthumb_trx_addons_sidebar_present' ) ) {
	add_filter( 'greenthumb_filter_sidebar_present', 'greenthumb_trx_addons_sidebar_present' );
	function greenthumb_trx_addons_sidebar_present($present) {
		return !is_single() && greenthumb_is_team_page() ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'greenthumb_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'greenthumb_trx_addons_thumb_size', 10, 2);
	function greenthumb_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = greenthumb_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('greenthumb_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'greenthumb_trx_addons_meta_box_fields', 10, 2);
	function greenthumb_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'greenthumb'),
				"desc" => wp_kses_data( __("Team member's email", 'greenthumb') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
// Hide unused hover menu styles
if ( !function_exists( 'greenthumb_filter_get_list_menu_hover' ) ) {
	add_filter( 'trx_addons_filter_get_list_menu_hover', 'greenthumb_filter_get_list_menu_hover');
	function greenthumb_filter_get_list_menu_hover($list) {
		unset($list['fade_box']);
