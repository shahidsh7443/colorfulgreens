	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_action_item_link_classes', 'sc_action_item_link sc_button sc_button_size_normal')); ?>"><?php 
